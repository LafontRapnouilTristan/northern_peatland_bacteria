...
df_sites_low_ubi <-
df2%>%
  filter(prev_category=="75<")%>% #keep only high prev ASV
  mutate(low_ubi=ifelse(totrelab<10,"low","high"))%>% # tag sites as high or low ubiquists relative abundance
...
low_ubi_genera_relab <- tmp_trans$plot_bar(facet = "low_ubi",barwidth = 1)+
  theme(axis.text.x = element_blank())
```
...
##Fig supp 1
```{r}
ggsave("suppl/sites_low_ubiquitous.png",low_ubi_plot,height = 4,width = 6.5)
```

##Fig supp 2
```{r}
ggsave("suppl/low_ubi_genera_relab.png",low_ubi_genera_relab,height = 4,width = 6.5)
```
##Fig supp 3
```{r}
ggsave("suppl/bxp_enz.png",enz_climate_bxp,height = 4)
```
##Fig supp 4
```{r}
ggsave("suppl/vpenz.png",gg,width = 9,height =5)
```

#VI- Save Environment and Session Info
```{r}
